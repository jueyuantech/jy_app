...
static void * fs_open(lv_fs_drv_t * drv, const char * path, lv_fs_mode_t mode)
{
    ...
    size_t idx = 0;
    char base_from_exe[MAX_PATH] = {0};
    const char base_macro[] = LV_FS_WIN32_PATH;
    const char *base_ptr = base_macro;
    bool use_exe_dir = false;
    if(base_macro[0] == '\0' || (base_macro[0] == '.' && base_macro[1] == '\0')) {
        use_exe_dir = true;
    }
    if(use_exe_dir) {
        DWORD n = GetModuleFileNameA(NULL, base_from_exe, (DWORD)sizeof(base_from_exe));
        if(n > 0 && n < sizeof(base_from_exe)) {
            for(int i = (int)n - 1; i >= 0; --i) {
                if(base_from_exe[i] == '\\' || base_from_exe[i] == '/') {
                    base_from_exe[i] = '\0';
                    break;
                }
            }
            base_ptr = base_from_exe;
        }
        else {
            base_ptr = ".";
        }
    }
    for(size_t i = 0; base_ptr[i] != '\0' && idx < sizeof(buf) - 1; i++) {
        buf[idx++] = base_ptr[i];
    }
    if(idx > 0 && buf[idx - 1] != '\\' && buf[idx - 1] != '/') {
        if(idx < sizeof(buf) - 1) buf[idx++] = '\\';
    }
    if(p) {
        while(*p && idx < sizeof(buf) - 1) {
            char c = *p++;
            if(c == '/') c = '\\';
            buf[idx++] = c;
        }
    ...
    DWORD abslen = GetFullPathNameA(buf, (DWORD)sizeof(absbuf), absbuf, NULL);
    if(abslen == 0 || abslen >= sizeof(absbuf)) LV_LOG_WARN("win32 fs try open: %s", buf);
    else LV_LOG_WARN("win32 fs try open: %s", absbuf);

    DWORD disposition = OPEN_EXISTING;
    if(mode & LV_FS_MODE_WR) disposition = OPEN_ALWAYS;
    ...
    if(h == INVALID_HANDLE_VALUE) {
        ...
        if((mode & LV_FS_MODE_WR) && (e == ERROR_PATH_NOT_FOUND)) {
            make_parent_dirs(buf);
            h = CreateFileA(
                    buf,
                    desired_access,
                    FILE_SHARE_READ | FILE_SHARE_WRITE,
 ...
 * @param drv       pointer to a driver where this function belongs
 * @param path      path to the file/directory beginning with the driver letter (e.g. S:/folder/file.txt)
...
static void make_parent_dirs(const char * path)
{
    char tmp[MAX_PATH];
    size_t n = 0;
    for(; path[n] && n < sizeof(tmp) - 1; n++) tmp[n] = path[n];
    tmp[n] = '\0';
    size_t last = n;
    while(last > 0) {
        if(tmp[last - 1] == '\\' || tmp[last - 1] == '/') break;
        last--;
    }
    if(last == 0) return;
...
static void * fs_dir_open(lv_fs_drv_t * drv, const char * path)
{
    ...
    /* Build normalized search pattern: <base>\path\* */
    char buf[MAX_PATH_LEN];
    const char *p = path;
    if(p && (p[0] == '/' || p[0] == '\\')) p++;
    size_t idx = 0;
    /* base path: LV_FS_WIN32_PATH or exe directory when empty/dot */
    char base_from_exe[MAX_PATH_LEN] = {0};
    const char base_macro[] = LV_FS_WIN32_PATH;
    const char *base_ptr = base_macro;
    bool use_exe_dir = false;
    if(base_macro[0] == '\0' || (base_macro[0] == '.' && base_macro[1] == '\0')) {
