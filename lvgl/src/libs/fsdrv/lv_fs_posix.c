 ...
 * @param drv       pointer to a driver where this function belongs
 * @param path      path relative to LV_FS_POSIX_PATH (without drive letter)
 */
static lv_fs_res_t fs_remove(lv_fs_drv_t * drv, const char * path)
{
    LV_UNUSED(drv);
    char buf[256];
    lv_snprintf(buf, sizeof(buf), LV_FS_POSIX_PATH "%s", path);
    if(unlink(buf) == 0) return LV_FS_RES_OK;
    if(errno == EISDIR || errno == ENOTEMPTY || errno == EEXIST || errno == EPERM) {
        if(rmdir(buf) == 0) return LV_FS_RES_OK;
    }
    if(errno == ENOENT) return LV_FS_RES_NOT_EX;
    if(errno == EACCES || errno == EPERM) return LV_FS_RES_DENIED;
    return LV_FS_RES_FS_ERR;
}
